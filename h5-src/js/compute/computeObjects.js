...
function CRUN2() {
	...
	this.c = new Array(null,null,null,null);
	this.group = -1;   //group -1代表不属于群组
}


//线路,不包括结点,由结点连接,电流方向为from->to
function CIRCU() {
	this.eleIndex=0;				//线路电流编号,默认编号是地址编号
	this.pressure=0;		        //起点与终点的电势差(U0 - U1)
	this.resistance=0;		    	//电阻(一般>=0, <0代表无穷大,断路)
	this.elecDir=NORMALELEC;		//电流状态标记
	this.elec=0;     				//线路电流大小
	this.from=null; this.to=null;	//起点和终点结点
	this.dirFrom=0; this.dirTo=0;	//起点和终点结点的方向
	this.indexInGroup=0;			//在群组内的序号
}
CIRCU.prototype.ConvertWhenElecLessZero = function() {	//当电流负数时改为正数,并调转电流方向
...
ROAD_Copy = function(newRoad, srcRoad) {
	var temp, now, pre;
...
//判断是否有结点point
ROAD.prototype.HaveRoadPoint = function(point) {
	var now = this.first;
	while (now) {
...
//判断是否有from->to路径
ROAD.prototype.HaveRoadStep = function(from, to) {
	if (!this.first)
		return false;
	var pre = this.first;
	var now = pre.next;
...
//在最后加入结点
ROAD.prototype.InsertPointAtTail = function(crunIndex) {
	var now;

	if (this.first) {
		this.last.next = now = new ROADSTEP();
		now.pre = this.last;
		this.last = now;
		now.crunIndex = crunIndex;
		now.next = null;
	} else {
		this.first = this.last = now = new ROADSTEP();
		now.crunIndex = crunIndex;
...
*/
	...
	this.size = size;
	this.circuitCount = 0;
	this.crunOrderMap = new Array(size);
	this.direct = new Array(bufSize);
	this.firstCircuit = new Array(bufSize);
	this.dir = new Array(bufSize);
}


function Equation(/*int */crunCount, /*int */eleCount) {
/*private:
...
/*const double **/Equation.prototype.GetAnswer = function()
//获得方程解的数组
{
	return this.x;
};

Equation.prototype.InputARow = function(/*const double * */buf)
...
Equation.prototype.OutputToFile = function()
//将方程保存到文件,测试函数
{
	/*int*/var i, j;
...
/*ELEC_STATE*/Equation.prototype.Count = function()
{
