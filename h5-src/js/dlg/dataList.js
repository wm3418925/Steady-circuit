...
// 处理类型属性
function ENUM_DATA_HANLDER(enumType, memeberName) {
	switch (enumType) {
	case CTRL_TYPE_ENUM:	//控件类型
		this.noteList = CTRL_TYPE_NAMES;
		break;
	case LEAD_STYLE_ENUM:	//导线样式
		this.noteList = LEAD_STYLE_NAMES;
		break;
	}
...
ENUM_DATA_HANLDER.prototype.GetMemeberName = function() {
	return this.memeberName;
};

ENUM_DATA_HANLDER.prototype.GetOptionCount = function() {
	return this.noteList.length;
};

ENUM_DATA_HANLDER.prototype.GetOptionNoteList = function() {
...
function LISTDATA() {
	...
	this.dataTypeList = new Array();		//每个数据项类型
	this.dataMinList = new Array();		//如果是short,int,long等整型数据,有最小值
	this.dataMaxList = new Array();		//如果是short,int,long等整型数据,有最大值
	this.memberNameList = new Array();	//数据
	this.noteTextList = new Array();		//每个数据项类型提示信息
}

LISTDATA.prototype.SetDataParent = function(dataParent) {
	this.dataParent = dataParent;
};

LISTDATA.prototype.Unint = function() {
...
LISTDATA.prototype.GetListSize = function() {
	return this.memberNameList.length;
};
LISTDATA.prototype.GetRowData = function(row) {
	var value;
...
LISTDATA.prototype.SetAMember = function(dataType, note, data, min/*1*/, max/*0*/) {
...
LISTDATA.prototype.SetAEnumMember = function(enumType, note, data, min/*1*/, max/*0*/) {
	if (min == undefined || max == undefined) {
		min = 1;
		max = 0;
	}
	...
	this.dataMaxList.push(max);
	this.noteTextList.push(note);
	this.memberNameList.push(new ENUM_DATA_HANLDER(enumType, data));
};

//检查一行: row--行, chData--字符串数据, enumData--枚举数据
LISTDATA.prototype.CheckAMember = function(row, pageElement) {
	var chData;
	var enumData;
...
LISTDATA.prototype.SaveAMember = function(row, pageElement) {
